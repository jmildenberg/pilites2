...
class TestGetPlay:
    def test_returns_full_play(self, client: TestClient) -> None:
        resp = client.get("/api/plays/play-1")
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_not_found_returns_404(self, client: TestClient) -> None:
        resp = client.get("/api/plays/ghost")
...
class TestCreatePlay:
    def test_create_new_play(self, client: TestClient) -> None:
        ...
        resp = client.post("/api/plays", json=payload)
        assert resp.status_code == 200
        assert resp.json()["ok"] is True
        assert client.get("/api/plays/play-2").status_code == 200
...
class TestUpdatePlay:
    def test_update_play(self, client: TestClient) -> None:
        play = client.get("/api/plays/play-1").json()
        play["name"] = "Renamed"
        resp = client.put("/api/plays/play-1", json=play)
        assert resp.status_code == 200
        assert client.get("/api/plays/play-1").json()["name"] == "Renamed"

    def test_update_nonexistent_returns_404(self, client: TestClient) -> None:
        payload = {"id": "ghost", "name": "Ghost", "regions": [], "cues": []}
        resp = client.put("/api/plays/ghost", json=payload)
        assert resp.status_code == 404


class TestDeletePlay:
    def test_delete_play(self, client: TestClient) -> None:
        resp = client.delete("/api/plays/play-1")
        assert resp.status_code == 200
        assert client.get("/api/plays/play-1").status_code == 404

    def test_delete_nonexistent_returns_404(self, client: TestClient) -> None:
        resp = client.delete("/api/plays/ghost")
        assert resp.status_code == 404


class TestRegionTest:
    def test_region_test_ok(self, client: TestClient) -> None:
        resp = client.post("/api/plays/play-1/regions/r-1/test")
        assert resp.status_code == 200

    def test_region_test_unknown_play(self, client: TestClient) -> None:
        resp = client.post("/api/plays/ghost/regions/r-1/test")
        assert resp.status_code == 404

    def test_region_test_unknown_region(self, client: TestClient) -> None:
        resp = client.post("/api/plays/play-1/regions/ghost/test")
        assert resp.status_code == 404
