...
class TestFadeIn:
    def test_at_zero_is_black(self) -> None:
        pixels = fade_in({"color": "#ffffff", "durationSec": 1.0}, 0.0, PIXEL_COUNT)
        assert_valid_output(pixels)
        assert all(p == (0, 0, 0) for p in pixels)

    def test_at_full_duration_is_full_color(self) -> None:
        pixels = fade_in({"color": "#ffffff", "durationSec": 1.0}, 1.0, PIXEL_COUNT)
        assert_valid_output(pixels)
        assert all(p == (255, 255, 255) for p in pixels)

    def test_past_duration_holds(self) -> None:
        p1 = fade_in({"color": "#ff0000", "durationSec": 1.0}, 1.0, PIXEL_COUNT)
        p2 = fade_in({"color": "#ff0000", "durationSec": 1.0}, 5.0, PIXEL_COUNT)
        assert p1 == p2

    def test_offset_delays_start(self) -> None:
        pixels = fade_in(
            {"color": "#ffffff", "durationSec": 1.0, "offsetSec": 2.0}, 1.0, PIXEL_COUNT
        )
        assert all(p == (0, 0, 0) for p in pixels)
...
class TestFadeOut:
    def test_at_zero_is_full_color(self) -> None:
        pixels = fade_out({"fromColor": "#ffffff", "durationSec": 1.0}, 0.0, PIXEL_COUNT)
        assert_valid_output(pixels)
        assert all(p == (255, 255, 255) for p in pixels)

    def test_at_full_duration_is_black(self) -> None:
        pixels = fade_out({"fromColor": "#ffffff", "durationSec": 1.0}, 1.0, PIXEL_COUNT)
        assert_valid_output(pixels)
        assert all(p == (0, 0, 0) for p in pixels)

    def test_past_duration_holds_black(self) -> None:
        p1 = fade_out({"fromColor": "#ffffff", "durationSec": 1.0}, 1.0, PIXEL_COUNT)
        p2 = fade_out({"fromColor": "#ffffff", "durationSec": 1.0}, 99.0, PIXEL_COUNT)
        assert p1 == p2


class TestStaticColor:
    def test_all_pixels_same_color(self) -> None:
        pixels = static_color({"color": "#ff0000", "intensity": 1.0}, 0.0, PIXEL_COUNT)
        assert_valid_output(pixels)
        assert all(p == (255, 0, 0) for p in pixels)

    def test_intensity_scales_color(self) -> None:
        pixels = static_color({"color": "#ffffff", "intensity": 0.5}, 0.0, PIXEL_COUNT)
        assert all(p[0] == 127 for p in pixels)

    def test_zero_intensity_is_black(self) -> None:
        pixels = static_color({"color": "#ffffff", "intensity": 0.0}, 0.0, PIXEL_COUNT)
        assert all(p == (0, 0, 0) for p in pixels)


class TestColorWash:
    def test_output_length(self) -> None:
        pixels = color_wash({"color": "#ffffff"}, 1.0, PIXEL_COUNT)
        assert_valid_output(pixels)

    def test_varies_over_time(self) -> None:
        p1 = color_wash({"color": "#ffffff", "speed": 1.0}, 0.0, PIXEL_COUNT)
        p2 = color_wash({"color": "#ffffff", "speed": 1.0}, 0.25, PIXEL_COUNT)
...
class TestGradient:
    def test_start_pixel_is_start_color(self) -> None:
        pixels = gradient(
            {"startColor": "#ff0000", "endColor": "#0000ff"}, 0.0, PIXEL_COUNT
        )
        assert_valid_output(pixels)
...
class TestStrobe:
    def test_on_during_duty(self) -> None:
        pixels = strobe({"color": "#ffffff", "rate": 1.0, "dutyCycle": 1.0}, 0.5, PIXEL_COUNT)
        assert all(p == (255, 255, 255) for p in pixels)

    def test_off_outside_duty(self) -> None:
        pixels = strobe({"color": "#ffffff", "rate": 1.0, "dutyCycle": 0.0}, 0.5, PIXEL_COUNT)
        assert all(p == (0, 0, 0) for p in pixels)

    def test_output_length(self) -> None:
        pixels = strobe({}, 0.0, PIXEL_COUNT)
        assert_valid_output(pixels)


class TestRainbow:
    def test_output_length(self) -> None:
        pixels = rainbow({}, 0.0, PIXEL_COUNT)
        assert_valid_output(pixels)

    def test_pixels_vary_across_strip(self) -> None:
        pixels = rainbow({"intensity": 1.0}, 0.0, PIXEL_COUNT)
        assert len(set(pixels)) > 1  # multiple distinct colors


class TestChase:
    def test_output_length(self) -> None:
        pixels = chase({}, 0.0, PIXEL_COUNT)
        assert_valid_output(pixels)

    def test_window_moves_over_time(self) -> None:
        p1 = chase({"speed": 1.0}, 0.0, PIXEL_COUNT)
        p2 = chase({"speed": 1.0}, 0.5, PIXEL_COUNT)
        assert p1 != p2


class TestPulse:
    def test_output_length(self) -> None:
        pixels = pulse({}, 0.0, PIXEL_COUNT)
        assert_valid_output(pixels)

    def test_all_same_color(self) -> None:
        pixels = pulse({"color": "#ff0000"}, 0.0, PIXEL_COUNT)
        assert len(set(pixels)) == 1


class TestLightning:
    def test_output_length(self) -> None:
        pixels = lightning({}, 0.0, PIXEL_COUNT)
        assert_valid_output(pixels)

    def test_deterministic(self) -> None:
        params = {"flashColor": "#ffffff", "strikeRate": 60.0}
        p1 = lightning(params, 1.5, PIXEL_COUNT)
        p2 = lightning(params, 1.5, PIXEL_COUNT)
        assert p1 == p2


class TestTwinkle:
    def test_output_length(self) -> None:
        pixels = twinkle({}, 0.0, PIXEL_COUNT)
        assert_valid_output(pixels)

    def test_varies_across_pixels(self) -> None:
        pixels = twinkle({"density": 0.5}, 0.0, PIXEL_COUNT)
        assert len(set(pixels)) > 1

    def test_deterministic(self) -> None:
        params = {"density": 0.3, "speed": 1.0}
        p1 = twinkle(params, 1.0, PIXEL_COUNT)
        p2 = twinkle(params, 1.0, PIXEL_COUNT)
        assert p1 == p2
